...
<head>
    ...
    <title>New Reminder Added</title>
</head>
    ...
    <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:16px;padding:32px;box-shadow:0 12px 30px rgba(15,23,42,0.08);">
        ...
        <p style="margin:0 0 24px;font-size:16px;color:#6b7280;">
            ...
            @if ($recipient->id === $creator->id)
                you just added a new reminder to <strong>{{ $family->name }}</strong>.
            @else
                <strong>{{ $creator->name }}</strong> added a new reminder to <strong>{{ $family->name }}</strong>.
            @endif
        </p>
        ...
        <div style="margin-bottom:24px;padding:20px;border-radius:12px;background:{{ $colors['bg'] }};border:1px solid {{ $colors['border'] }};">
            ...
            <p style="margin:0 0 4px;font-size:14px;color:#6b7280;">{{ $nextDate }}</p>

            @if ($reminder->recurs_yearly)
                <p style="margin:0 0 8px;font-size:12px;color:#9ca3af;">Recurs every year &nbsp;·&nbsp; Notify {{ $reminder->remind_days_before }} day(s) before</p>
            @else
                <p style="margin:0 0 8px;font-size:12px;color:#9ca3af;">One-time reminder &nbsp;·&nbsp; Notify {{ $reminder->remind_days_before }} day(s) before</p>
            @endif
